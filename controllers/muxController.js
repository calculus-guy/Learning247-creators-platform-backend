...
const Video = require('../models/Video');
// Ensure the file path matches exactly what is on your server (case-sensitive on Linux)
const LiveClass = require('../models/liveClass'); 

// Access the Webhooks static utility directly
...
exports.handleMuxWebhook = async (req, res) => {
  ...
  let event;
  try {
    // 2. THE FIX: Do NOT use 'new Webhooks()'.
    // We pass the raw body and headers directly to the static verifySignature method.
    // Note: We pass req.body directly (it should be a Buffer from the raw middleware).
    
    // Verify signature returns true if valid, or throws an error if invalid
    Webhooks.verifySignature(req.body, req.headers, endpointSecret);

    // 3. Parse the body only AFTER verification succeeds
  ...
  try {
    ...
    if (type === 'video.asset.ready' && data.upload_id && !data.live_stream_id) {
      ...
      if (!uploadId) {
        return res.status(200).send('Skipped: Missing upload_id');
      }
    ...
    console.error('[Webhook] Error processing event logic:', err);
    return res.status(500).send('Internal server error while processing webhook.');
  }
};